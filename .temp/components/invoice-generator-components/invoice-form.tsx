...
const InvoiceForm: React.FC<{ currentDate?: string }> = ({ currentDate }) => {
    ...
    const handleCalculateTotal = useCallback(() => {
        let newSubTotal: Decimal = items.reduce(
            (acc, item) =>
                acc.plus(new Decimal(item.price).times(item.quantity)),
            new Decimal(0)
        );
        let newtaxAmount: Decimal = newSubTotal.times(
            final.taxRate.dividedBy(100)
        );
                    ...
                    <CardHeader className="flex flex-row items-start justify-between">
                        <div className="flex flex-col">
                            ...
                            <div className="flex flex-row items-center">
                                <span className="me-2 block font-bold">
                                    Due&nbsp;Date:
                                </span>
                                <Input
                                    type="date"
                            ...
                            <div className="flex flex-col space-y-2">
                                <div className="flex flex-row items-start justify-between">
                                    ...
                                    <span>
                                        {modifiers.currency}
                                        {Math.round(
                                            formatDecimal(final.subTotal) * 100
                                        ) / 100}
                                    </span>
                                </div>
                                <div className="flex flex-row items-start justify-between">
                                ...
                                <div className="flex flex-row items-start justify-between">
                                    ...
                                    <span>
                                        ...
                                        {modifiers.currency}
                                        {Math.round(
                                            formatDecimal(final.taxAmount) * 100
                                        ) / 100 || 0}
                                    </span>
                                </div>
                                <hr />
